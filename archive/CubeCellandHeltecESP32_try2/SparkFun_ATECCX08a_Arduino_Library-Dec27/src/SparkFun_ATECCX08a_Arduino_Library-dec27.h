...
/* Receive constants */
#define ATRCC508A_MAX_REQUEST_SIZE 96
...
class ATECCX08A {
  public:
	...
	#else
 // I changed SerialUSB to serial "Stream &serialPort = SerialUSB" == > "Stream &serialPort = Serial"
	...
	boolean createSignature(uint8_t *data, uint16_t slot = 0x0000, uint8_t *signature);
	boolean loadTempKey(uint8_t *data);  // load 32 bytes of data into tempKey (a temporary memory spot in the IC)
	boolean signTempKey(uint16_t slot = 0x0000, uint8_t *signature); // create signature using contents of TempKey and PRIVATE KEY in slot
