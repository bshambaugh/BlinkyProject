...
#include "SparkFun_ATECCX08a_Arduino_Library-dec27.h"
...
boolean ATECCX08A::createSignature(uint8_t *data, uint16_t slot,uint8_t *signature)
{
  if (!loadTempKey(data) || !signTempKey(slot,signature))
    return false;
...
boolean ATECCX08A::signTempKey(uint16_t slot,uint8_t *signature)
{
